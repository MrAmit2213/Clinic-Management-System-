...
const Reducer = (state = initState, action) => {
    switch (action.type) {
        ...
        case 'ADMIN_LOGIN':
            return { ...state, userId: action.userId, fName: action.fName, lName: action.lName, email: action.email, number: action.number, gender: action.gender, image: action.image, isLoggedIn: true, user: action.user }
        case 'ADMIN_LOGOUT':
            return { ...state, userId: '', fName: '', lName: '', email: '', number: '', gender: '', image: '', isLoggedIn: false, user: '' }
        default:
            return state;
